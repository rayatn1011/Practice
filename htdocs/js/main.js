$(document).ready(function () {
    ...
    $('#add-todo').click(function(e){
        ...
        $.post('todo/create.php',{todo: todoItem} , function(data){
            ...
            if(data.status=='success') {
                ...
                var cloneTodo = $('<li>').append('<input type="checkbox">').append('<span class="content">').append('<input type="text" value="todo item">').append('<button class="todo-btn" data-action="delete">&times;</button>').append('<hr>');
    ...
    $('#todo-list').on('blur', '[type="text"]', function (e) {
        ...
        $.post('todo/update.php',{id:123,todo: content},function(data){
            if(data.status=='success') {
    ...
    $.ajax({
        ...
        success: function (data) {
            $.each(data.results, function (index, user) {
                ...
                //  insert DOM object into DOM
                // .append(imageTag).append(nameTag).append(emailTag)
                $('.practice-api-user').append(imageTag).append(nameTag).append(emailTag);
